...
namespace UnityEngine.U2D.Common.UTess
{
	...
	internal struct ModuleHandle
	{
		...
		internal unsafe static void InsertionSort<T, U>(void* array, int lo, int hi, U comp) where T : struct
		{
		}

		internal static int GetLower<T, U, X>(NativeArray<T> values, int count, U check, X condition) where T : struct where U : struct
		{
			return 0;
		}

		internal static int GetUpper<T, U, X>(NativeArray<T> values, int count, U check, X condition) where T : struct where U : struct
		{
			return 0;
		}

		internal static int GetEqual<T, U, X>(Array<T> values, int count, U check, X condition) where T : struct where U : struct
		{
			return 0;
		}

		internal static int GetEqual<T, U, X>(NativeArray<T> values, int count, U check, X condition) where T : struct where U : struct
		{
			return 0;
		}
		...
		{
			outVertexCount = default(int);
			outIndexCount = default(int);
			outEdgeCount = default(int);
			return default(float4);
		}
